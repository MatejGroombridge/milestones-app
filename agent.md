...
> **Naming in this repo.** The patterns below are written against Habit
> Tracker, the app they were first evolved in. Milestones implements the
> same three under its own names — `MilestoneCard`,
> `MilestoneOverviewDialog` and `MilestoneEditorDialog`, plus a fourth,
> `LogRecordDialog`, for the tap-a-card primary action. The shared
> scaffolding (`EditorSection`, `CaptionedSection`, `ChoiceCell`,
> `CompactStepper`, `IconBadge`, the icon+colour picker) lives in
> `ui/components/EditorPieces.kt` rather than being duplicated privately
> inside each dialog.
...
| `celebrateRecords = false` | Confetti burst on a new personal best | The record itself still saves |
| `zenMode = true` | Bottom nav, FAB, top-bar archive icon, the overview + editor dialogs, the History and Progress pages, every Settings section except the Zen toggle | Tap-a-card → `LogRecordDialog` on the Milestones grid + the ability to disable Zen via Settings |

Note that Zen mode here keeps *one* dialog reachable rather than none.
Logging a record needs a value, so the primary action can't collapse to
a bare tap the way "mark this habit done" could — the mode strips
